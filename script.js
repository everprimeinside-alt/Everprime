...
onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
        ...
        // როცა მომხმარებელი შესულია, ღილაკი აჩვენებს პროფილის აიქონს
        if(navUser) {
            navUser.innerHTML = `
                <button onclick="window.toggleProfile()" class="nav-btn flex items-center justify-center p-2.5 rounded-none border border-[#FF0000]/40 hover:border-[#FF0000] transition-all bg-black/50 text-white group" title="პროფილი">
                    <span class="material-icons text-xl text-[#FF0000] group-hover:scale-10 transition-transform">person</span>
                </button>`;
        }
...
const mainImg = (p.images && p.images.length > 0) ? p.images[0] : (p.image || 'logo.jpg');
                grid.innerHTML += `
            <div class="product-card group flex flex-col h-full p-2.5 ${!inStock ? 'opacity-80' : ''}">
                <div class="flex-grow">
                    <div class="relative h- w-full flex items-center justify-center bg-black/40 mb-3 border border-white/5 overflow-hidden">
                        <span style="border-radius:500px" class="absolute top-2 left-2 px-2 py-1 text-[9px] font-bold uppercase z-10 ${inStock ? 'bg-green-600 text-white' : 'bg-red-600 text-white'}">
                    ...
                    <div class="flex justify-between items-start mb-3">
                        <div>       <p class="text-red-600 font-bold text-lg">${p.price}₾</p>
                ...
                <!-- სამივე ღილაკი იდენტური ზომითა და ჰორიზონტალური განლაგებით -->
                <div class="mt-auto grid grid-cols-3 gap-1">
                    <button onclick="window.showDetails('${p.id}')" class="details-btn h-10 flex items-center justify-center p-0 m-0" title="დეტალები">
                        <span class="material-icons text-base text-gray-300">visibility</span>
                    </button>
                    <button onclick="window.shareProduct('${p.id}')" class="details-btn h-10 flex items-center justify-center p-0 m-0" title="გაზიარება">
                        <span class="material-icons text-base text-red-600">share</span>
                    </button>
                    <button ${inStock ? `onclick="window.order('${p.id}')"` : 'disabled'} class="details-btn h-10 flex items-center justify-center p-0 m-0 ${!inStock ? 'opacity-50 cursor-not-allowed' : ''}" title="${inStock ? 'შეკვეთა' : 'ამოიწურა'}">
                        <span class="material-icons text-base ${inStock ? 'text-white' : 'text-gray-500'}">${inStock ? 'shopping_cart' : 'block'}</span>
                    </button>
                </div>
            </div>`;
    });
     ...
     content.innerHTML = `
        <div class="flex flex-col gap-6">
            ...
            <div class="text-left">
                ...
                <!-- სლაიდშოუ შენარჩუნებულია, ხოლო ღილაკები ჰორიზონტალურად და აიქონებითაა -->
                <div class="grid grid-cols-3 gap-2 w-full">
                    <button ${inStock ? `onclick="window.order('${p.id}'); window.closeDetails()"` : 'disabled'} class="details-btn h-12 flex items-center justify-center p-0 m-0 ${!inStock ? 'opacity-50 cursor-not-allowed' : ''}" title="${inStock ? 'შეკვეთა' : 'ამოიწურა'}">
                        <span class="material-icons text-xl ${inStock ? 'text-white' : 'text-gray-500'}">${inStock ? 'shopping_cart' : 'block'}</span>
                    </button>
                    
                    <button onclick="window.shareProduct('${p.id}')" class="details-btn h-12 flex items-center justify-center p-0 m-0 bg-white/10 hover:bg-white/20 transition-all" title="გაზიარება">
                        <span class="material-icons text-xl text-red-600">share</span>
                    </button>
                    
                    <button onclick="window.closeDetails()" class="details-btn h-12 flex items-center justify-center p-0 m-0 bg-white/10 hover:bg-white/20 transition-all" title="დახურვა">
                        <span class="material-icons text-xl text-gray-300">close</span>
                    </button>
                </div>

                <p class="text-gray-400 text-xs leading-relaxed border-l-2 border-red-600 pl-4 mb-6 whitespace-pre-line mt-4">${p.desc || 'აღწერა არ არის.'}</p>
            </div>
        </div>
    `;
